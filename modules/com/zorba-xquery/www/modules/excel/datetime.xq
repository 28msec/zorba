...
(:~
 : This is a library module offering the same set of functions defined by
 : Microsoft Excel, under Date and Time Functions.
 :
 : @author Sorin Nasoi
 : @version 1.0
 :
 : @see <a href="http://office.microsoft.com/en-us/excel/CH062528231033.aspx">Excel Documentation: Date and Time Functions </a>
 :
 :)
...
(:~
 : Helper function: returns the day of the week as a number,
 : where from 0 (Sunday) to 6 (Saturday), given a $date.
 :
 : @param $date the date
 :)
...
(:~
 : Helper function: returns true if the year part of the supplied $date is a
...
(:~
 ...
 : The metod used is U.S. (NASD). If the starting date is the last day of a
 : month, it becomes equal to the 30th of the same month.
 : If the ending date is the last day of a month and the starting date is
 : earlier than the 30th of a month, the ending date becomes equal to the 1st of
 : the next month; otherwise the ending date becomes equal to the 30th of the
 : same month.
 :
...
(:~
 ...
 : changed to the 30th;
 : - Where date B falls on the last day of February, the actual date B will
 : be used.
 :
 :The US/NASD Method (30US/360)
 : - If both date A and B fall on the last day of February, then date B will be
 : changed to the 30th.
 : - If date A falls on the 31st of a month or last day of February, then
 : date A will be changed to the 30th.
 : - If date A falls on the 30th of a month after applying (2) above and
 : date B falls on the 31st of a month,
 : then date B will be changed to the 30th.
 :
 : @see http://office.microsoft.com/en-us/excel/HP052090471033.aspx
 : @param $start_date the start date
 : @param $end_date the end date
 : @param $method if false then US/NASD Method is used, otherwise
 : the European Method is used.
 :)
declare function excel-datetime:days360
  ( $start_date as xs:date,
    $end_date   as xs:date,
  ...
  if($method eq fn:true()) then (: European Method :)
    let $sd :=  if (fn:day-from-date($start_date) eq 31) then
      excel-datetime:date(fn:year-from-date($start_date),
                          fn:month-from-date($start_date), 30)
      else $start_date

    let $ed :=  if (fn:day-from-date($end_date) eq 31) then
      excel-datetime:date(fn:year-from-date($end_date),
                          fn:month-from-date($end_date), 30)
      else $end_date

    let $ed1 := excel-datetime:date(fn:year-from-date($ed),
                                    fn:month-from-date($ed),
                                    01)
    ...
    let $ed :=  if(($start_date = $end_date) and
       ((excel-datetime:is-leap-year($end_date) and
        (fn:day-from-date($end_date) eq 29)) or
    ...
    return if((fn:day-from-date($start_date) eq 31) or
      ((excel-datetime:is-leap-year($start_date) and
       (fn:day-from-date($start_date) eq 29)) or
      (fn:day-from-date($start_date) eq 28))) then
      let $sd :=  excel-datetime:date(fn:year-from-date($start_date),
                                      fn:month-from-date($start_date), 30)
      let $ed := if (fn:day-from-date($ed) eq 31) then
      excel-datetime:date(fn:year-from-date($ed),
                          fn:month-from-date($ed), 30) else
      $ed
      let $ed1 := excel-datetime:date(fn:year-from-date($ed),
                                      fn:month-from-date($ed), 01)
...
(:~
 ...
 : @param $date the date
 : @return  the month of a $date. The month is given as an integer,
...
(:~
 : Returns the seconds of a $time value. The second is given as an integer
 : in the range 0 (zero) to 59.
 :
...
(:~
 ...
 : @param $date the date
 : @param $return_type 1 for Numbers 1 (Sunday) through 7 (Saturday).
 : 2 for Numbers 1 (Monday) through 7 (Sunday).
 : 3 for Numbers 0 (Monday) through 6 (Sunday).
...
declare function excel-datetime:weekday
  ...
  if ($return_type eq 1) then
    ...
    fn:error( $excel-err:errValue,
              "Provided number must be in range [1,3]",
              $return_type)
};
...
(:~
 ...
 : Use NETWORKDAYS to calculate employee benefits that accrue based on the
 : number of days worked during a specific term.
 :
...
(:~
 ...
 : @param $start_date the start date
 : @param $end_date the end date
 : @param $holidays one or more dates to exclude from the working calendar,
...
declare function excel-datetime:networkdays
    ($start_date        as xs:date,
     ...
     + (if(excel-datetime:day-of-week($start_date) < 7) then
        6 - excel-datetime:day-of-week($start_date) else 0)
