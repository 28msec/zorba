...
$content := dml:apply-insert-nodes-last($coll, for $i  in 1 to 10 return <c>{$i}</c>);
local:check(dml:collection($coll)[position() > last() - 10]);

$content := dml:apply-insert-nodes-before($coll, dml:collection($coll)[2], for $i  in 1 to 10 return <d>{$i}</d>);
local:check($coll)[1 < position() and position() <= 11];

$content := dml:apply-insert-nodes-after($coll, dml:collection($coll)[2], for $i  in 1 to 10 return <e>{$i}</e>);
