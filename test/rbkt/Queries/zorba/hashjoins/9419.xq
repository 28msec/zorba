...
declare function local:match_filter_entry($qso_node, $filter_node)
{
	...
	return
		if (local:node-kind($filter_entry/node()) = "element") then
			...
			if (fn:empty($filter_entry/node())) then
				()
			else
			($qso_node/*[fn:local-name(.) eq fn:local-name($filter_entry)] eq $filter_entry, fn:local-name($filter_entry))
};
