...
namespace zorbac {
  ...
  StaticContext::assign_functions(XQC_StaticContext context)
  {
    ...
    //SMTP settings
    context->set_SMTP_server                   = StaticContext::set_SMTP_server;
    context->get_SMTP_server                   = StaticContext::get_SMTP_server;
    context->set_SMTP_uname                    = StaticContext::set_SMTP_uname;
    context->get_SMTP_uname                    = StaticContext::get_SMTP_uname;
    context->set_SMTP_upwd                     = StaticContext::set_SMTP_upwd;
    context->get_SMTP_upwd                     = StaticContext::get_SMTP_upwd;
  ...
  XQUERY_ERROR
  StaticContext::set_SMTP_server(XQC_StaticContext context, const char* SMTP_server )
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);

    lContext->setSMTPServer(SMTP_server);
    SC_CATCH
  }

  XQUERY_ERROR
  StaticContext::get_SMTP_server(XQC_StaticContext context, const char** SMTP_server)
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);;
    zorbac::StaticContext* lWrapper = static_cast<zorbac::StaticContext*>(context->data);

    zorba::String lSMTPServer = lContext->getSMTPServer();
    lWrapper->theStrings.push_back(lSMTPServer);

    (*SMTP_server) = lSMTPServer.c_str();
    SC_CATCH
  }

  XQUERY_ERROR
  StaticContext::set_SMTP_uname(XQC_StaticContext context, const char* SMTP_uname )
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);

    lContext->setSMTPUname(SMTP_uname);
    SC_CATCH
  }

  XQUERY_ERROR
  StaticContext::get_SMTP_uname(XQC_StaticContext context, const char** SMTP_uname)
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);;
    zorbac::StaticContext* lWrapper = static_cast<zorbac::StaticContext*>(context->data);

    zorba::String lSMTPUsername = lContext->getSMTPUname();
    lWrapper->theStrings.push_back(lSMTPUsername);

    (*SMTP_uname) = lSMTPUsername.c_str();
    SC_CATCH
  }

  XQUERY_ERROR
  StaticContext::set_SMTP_upwd(XQC_StaticContext context, const char* SMTP_upwd )
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);

    lContext->setSMTPUpwd(SMTP_upwd);
    SC_CATCH
  }

  XQUERY_ERROR
  StaticContext::get_SMTP_upwd(XQC_StaticContext context, const char** SMTP_upwd)
  {
    SC_TRY
      zorba::StaticContext* lContext = getStaticContext(context);;
    zorbac::StaticContext* lWrapper = static_cast<zorbac::StaticContext*>(context->data);

    zorba::String lSMTPPassword = lContext->getSMTPUpwd();
    lWrapper->theStrings.push_back(lSMTPPassword);

    (*SMTP_upwd) = lSMTPPassword.c_str();
    SC_CATCH
  }
#endif  /* ZORBA_WITH_EMAIL */
