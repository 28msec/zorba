...
class ParseNodeXQDocVisitor : public parsenode_visitor {
...
ParseNodeXQDocVisitor(std::ostream &aStream)
  : theIndent(0),
    os(aStream)
{
}
...
BEGIN_END_TAG (IndexDecl)
BEGIN_END_TAG (IndexField)
BEGIN_END_TAG (IndexFieldList)
BEGIN_END_TAG (IndexStatement)
END_TAG (VarDecl)
END_TAG (VarGetsDecl)
...
void print_parsetree_xqdoc (ostream &os, const parsenode *p) {
  ParseNodeXQDocVisitor v (os);
  v.print (p);
}
