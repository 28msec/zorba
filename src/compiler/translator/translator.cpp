...
protected:
  ...
  xqpStringStore_t                     theModuleNamespace;
  xqpStringStore_t                     theModulePrefix;
...
{
  sctx_p->expand_qname(qnameItem,
                       sctx_p->default_function_ns(),
                       qname->get_prefix(),
                       qname->get_localname(),
                       loc);
}
...
var_expr_t create_var(
    ...
    xqtref_t type = NULL) 
{
  var_expr_t e = new var_expr(sctxid(), loc, kind, qname);
...
void bind_fn(
    ...
    const QueryLoc& loc) 
{
  sctx_p->bind_fn(qnameItem, f, nargs, loc);
...
function* lookup_fn(const QName* qname, ulong arity, const QueryLoc& loc)
{
  ...
  return sctx_p->lookup_fn(qnameItem, arity);
}
...
void* begin_visit(const DecimalFormatNode& v) 
{
  ...
  sctx_p->add_decimal_format(df, loc);
...
void* begin_visit(DefaultNamespaceDecl const& v) 
{
  ...
  case ParseConstants::ns_element_default:
    sctx_p->set_default_elem_type_ns(v.get_default_namespace(), loc);
    break;
  case ParseConstants::ns_function_default:
    sctx_p->set_default_function_ns(v.get_default_namespace(), loc);
    break;
  }
  return NULL;
}
...
void* begin_visit(const SchemaImport& v) 
{
  ...
  xqpStringStore_t targetNS = v.get_uri();
  ...
  if (prefix != NULL) 
  {
    ...
    xqpStringStore_t pfx = prefix->get_prefix();

    if (*pfx == "xml" || *pfx == "xmlns")
      ZORBA_ERROR_LOC(XQST0070, loc);

    if (prefix->get_default_bit()) 
      sctx_p->set_default_elem_type_ns(targetNS, loc);

    if (! pfx->empty())
      sctx_p->bind_ns(pfx, targetNS, loc, XQST0033);
  }

  store::Item_t targetNSItem = NULL;
  xqpStringStore_t tmp = targetNS;
...
void end_visit(const ModuleImport& v, void* /*visit_state*/) 
{
  ...
  if (pfx != NULL &&
      ! pfx->empty() && 
      ! (pfx->byteEqual(theModulePrefix) &&
         targetNS->byteEqual(theModuleNamespace)) ) 
  {
    sctx_p->bind_ns(pfx, targetNS, loc, XQST0033);
  }
  ...
  {
    standardModuleResolver->resolveTargetNamespace(targetNS->str(), *sctx_p, compURIs);
  }
  else
  {
    for (ulong i = 0; i < atlist->size(); ++i) 
       ...
       ++ite) 
  {
    ...
    else 
    {
      ...
      importedSctx->set_module_namespace(targetNS->str());
      ...
      xqpStringStore_t fileURL;
      if (compURL.size() != 0) 
      {
        fileURL = new xqpStringStore(compURL);
      }
      else
      {
        fileURL = new xqpStringStore(compURI);
      }
    ...
    sctx_p->import_module(importedSctx.getp(), loc);
...
void* begin_visit(const VFO_DeclList& v) 
{
       ...
       ++it)
  {
    ...
    case ParseConstants::fn_extern_sequential:
    {
      ...
      f = sctx_p->lookup_fn(qnameItem, nargs);
      ...
      try 
      {
        ef = sctx_p->lookup_stateless_external_function(qnameItem->getNamespace(),
...
void* begin_visit(const IntegrityConstraintDecl& v) 
{
  ...
  case IntegrityConstraintDecl::coll_check_simple:
  {
    ...
    **********************/
    ...
    const xqpStringStore_t& prefixStr = ic.getCollName()->get_prefix();
    xqpStringStore_t uriStr;
    sctx_p->lookup_ns(uriStr, prefixStr, loc);
  ...
  case IntegrityConstraintDecl::coll_check_unique_key:
  {
    ...
    **********************/
    ...
    // "org:employees"
    expr_t qnameStrExpr = new const_expr(sctxid(),
                                         loc,
                                         ic.getCollName()->get_qname());
  ...
  case IntegrityConstraintDecl::foreign_key:
  {
    ...
    **********************/
    ...
    const xqpStringStore_t& toPrefixStr = ic.getToCollName()->get_prefix();
    xqpStringStore_t toUriStr;
    sctx_p->lookup_ns(toUriStr, toPrefixStr, loc);
    ...
    const xqpStringStore_t& fromPrefixStr = ic.getFromCollName()->get_prefix();
    xqpStringStore_t fromUriStr;
    sctx_p->lookup_ns(fromUriStr, fromPrefixStr, loc);
    ...
    fo_expr_t fromCollExpr = new fo_expr(sctxid(),
                                         loc,
                                         fromFnCollection, 
                                         fromArgColl);
...
void end_visit(const BlockBody& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new sequential_expr(sctxid(), loc, stmts));
...
void end_visit(const UnionExpr& v, void* /*visit_state*/) 
{
  ...
  // and duplicate elimi
  push_nodestack(new fo_expr(sctxid(),
                             loc,
...
void end_visit (const InstanceofExpr& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new instanceof_expr(sctxid(),
                                     loc,
                                     pop_nodestack(),
                                     pop_tstack()));
}
...
void end_visit(const TreatExpr& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new treat_expr(sctxid(),
                                loc,
                                pop_nodestack(),
                                pop_tstack(),
                                XPDY0050));
...
void end_visit (const ValidateExpr& v, void* /*visit_state*/) 
{
  ...
  {
    const xqpStringStore_t& prefix = v.get_type_name()->get_prefix();
    xqpStringStore_t ns;
  ...
  push_nodestack(new validate_expr(sctxid(),
                                   loc,
                                   v.get_valmode(),
                                   qname,
                                   pop_nodestack(),
...
void end_visit(const Pragma& v, void* /*visit_state*/) 
{
  ...
  // may raise XPST0081
  xqpStringStore_t ns;
  sctx_p->lookup_ns(ns, v.get_name()->get_prefix(), loc);
}
  ...
  if ((axisExpr = dynamic_cast<axis_step_expr *>(top)) != NULL) 
  {
    ...
    else 
    {
      ...
      case ParseConstants::wild_elem:
      {
        ...
        xqpStringStore_t localname = new xqpStringStore(":wildcard");

        store::Item_t qnItem;

        if (axisExpr->getAxis() == axis_kind_attribute)
        {
          sctx_p->expand_qname(qnItem,
                               getEmptyString(),
                               wildcard->getPrefix(),
                               localname,
    ...
    else
    {
      ...
      switch (wildcard->getKind())
      {
        case ParseConstants::wild_all:
          ...
          break;
        case ParseConstants::wild_elem:
          cc->add_nametest_h(new NodeNameTest(NULL, wildcard->getPrefix()));
          break;
        case ParseConstants::wild_prefix:
          cc->add_nametest_h(new NodeNameTest(wildcard->getLocalName(), NULL));
          break;
      }
    }
  }
...
void end_visit(const NumericLiteral& v, void* /*visit_state*/) 
{
  ...
  {
    push_nodestack(new const_expr(sctxid(), loc, v.get<xqp_integer>()));
    break;
  }
  case ParseConstants::num_decimal: 
  {
    push_nodestack(new const_expr(sctxid(), loc, v.get<xqp_decimal>()));
    break;
  }
  case ParseConstants::num_double: 
  {
    push_nodestack(new const_expr(sctxid(), loc, v.get<xqp_double>()));
    break;
  }
  }
}
...
void end_visit(const VarRef& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new wrapper_expr(sctxid(), v.get_location(), rchandle<expr>(ve)));
}
...
void end_visit (const OrderedExpr& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new order_expr(sctxid(),
                                loc,
                                order_expr::ordered,
                                pop_nodestack()));
}
...
void end_visit(const FunctionCall& v, void* /*visit_state*/) 
{
  ...
  rchandle<QName> qname = v.get_fname();
  const xqpStringStore_t& localName = qname->get_localname();

  store::Item_t qnameItem;
  expand_function_qname(qnameItem, qname, loc);

  const xqpStringStore* fn_ns = qnameItem->getNamespace();
  ...
  if (fn_ns->byteEqual(XQUERY_FN_NS, strlen(XQUERY_FN_NS))) 
  {
    ...
    else if (*localName == "number") 
    {
      ...
      push_nodestack(&*wrap_in_let_flwor(new treat_expr(sctxid(),
                                                        loc,
                                                        data_expr,
      ...
      return;
             ...
             xquery_fns_def_dot.find(localName->str()) != xquery_fns_def_dot.end()) 
    {
      arguments.push_back(DOT_REF);
    }
    ...
    else if (*localName == "doc")
    {
      if (numArgs > 0) 
      {
        ...
        if(doc_uri->get_expr_kind() == const_expr_kind)
        {
          ...
          try
          {
            if(uri_string->bytePositionOf(":/", 3, 0) >= 0)
            {
              URI docURI(uri_string.getp(), true);//with validate
            }
          }
          catch(error::ZorbaError &e)
          {
  ...
  else if (fn_ns->byteEqual(ZORBA_OP_NS, strlen(ZORBA_OP_NS))) 
  {
    if (*localName == "inline-xml" && numArgs == 1) 
    {
      push_nodestack(new eval_expr(sctxid(),
                                   loc,
...
void end_visit(const DirElemConstructor& v, void* /*visit_state*/) 
{
  ...
  nameExpr = new const_expr(sctxid(), loc, qnameItem);

  push_nodestack(new elem_expr(sctxid(),
                               loc,
                               nameExpr,
                               attrExpr,
                               contentExpr,
...
void end_visit(const DirAttr& v, void* /*visit_state*/) 
{
  ...
  {
    xqpStringStore_t prefix = getEmptyString();

    if (qname->get_qname() != "xmlns") 
    {
      prefix = qname->get_localname();
      if (prefix->byteEqual("xmlns", 5))
        ZORBA_ERROR_LOC (XQST0070, loc);
    }
    ...
    if (constValueExpr != NULL) 
    {
      ...
      sctx_p->bind_ns(prefix, uri, loc, XQST0071);
      ns_ctx->bind_ns(prefix, uri);

      if (prefix->empty())
        sctx_p->set_default_elem_type_ns(uri, loc);
    }
    else if (valueExpr == NULL)
    {
      if (prefix->byteEqual("xml", 3))
        ZORBA_ERROR_LOC(XQST0070, loc);

      // unbind the prefix
      xqpStringStore_t empty1 = getEmptyString();
      xqpStringStore_t empty2 = getEmptyString();
      sctx_p->bind_ns(prefix, empty1, loc, XQST0071);
      ns_ctx->bind_ns(prefix, empty2);

      if (prefix->empty())
        sctx_p->set_default_elem_type_ns(empty1, loc);
    }
    else
    {
  ...
  else
  {
    ...
    expr_t nameExpr = new const_expr(sctxid(), loc, qnameItem);
...
void end_visit (const DirElemContent& v, void* /*visit_state*/) {
  ...
  if (v.get_direct_cons() != NULL) {
    ...
    if (!v.isStripped()) {
      ...
      push_nodestack (new text_expr(sctxid(), loc,
...
void end_visit (const CDataSection& v, void* /*visit_state*/) {
  ...
  push_nodestack (new text_expr (sctxid(), loc, text_expr::text_constructor, content));
}
...
void attr_val_content (const QueryLoc& loc, const CommonContent *cc, xqpString content)
{
  if (cc == NULL) 
  {
    push_nodestack(new const_expr (sctxid(), loc, content));
  }
  else
  {
...
void end_visit (const DirPIConstructor& v, void* /*visit_state*/) {
  ...
  push_nodestack (new pi_expr (sctxid(), loc, target,  content));
}
...
void end_visit (const CompDocConstructor& v, void* /*visit_state*/) 
{
  ...
  push_nodestack(new doc_expr(sctxid(), loc, lEnclosed));
}
...
void end_visit(const CompElemConstructor& v, void* /*visit_state*/) 
{
  ...
  push_nodestack (new elem_expr(sctxid(), loc, nameExpr, contentExpr, ns_ctx));
}
...
void end_visit(const AssignExpr& v, void* visit_state) 
{
  ...
  push_nodestack(new fo_expr(sctxid(), loc, var_set, qname_expr, pop_nodestack()));
}
...
{
  TRACE_VISIT_OUT();
  push_nodestack (new exit_expr(sctxid(), loc, pop_nodestack()));
}
...
void end_visit(const WhileExpr& v, void* visit_state) 
{
  ...
  push_nodestack(new while_expr(sctxid(), loc, seqBody));
}
...
void end_visit (const FlowCtlStatement& v, void* visit_state) 
{
  ...
  push_nodestack (new flowctl_expr (sctxid(), loc, a));
}
...
void end_visit (const FTCaseOption& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  push_ftstack( new ftcase_option_expr( v.get_location(), v.get_mode() ) );
}
...
void end_visit (const FTContainsExpr& v, void* /*visit_state*/) {
  ...
  ftcontains_expr *const e = new ftcontains_expr(
    sctxid(), v.get_location(), range, ftselection, ftignore
  );
  push_nodestack( e );
}
...
void end_visit (const FTDiacriticsOption& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  ftdiacritics_option_expr *const d =
    new ftdiacritics_option_expr( v.get_location(), v.get_mode() );
  push_ftstack( d );
}
...
void end_visit (const FTExtensionOption& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  ftextension_option_expr *const eo =
    new ftextension_option_expr( v.get_location(), v.get_qname(), v.get_val() );
  push_ftstack( eo );
}
...
void end_visit (const FTStopWords& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  ftstop_words_expr *const sw =
    new ftstop_words_expr( v.get_location(), v.get_uri(), v.get_stop_words() );
  push_ftstack( sw );
}
...
void end_visit (const FTStopWordsInclExcl& v, void* /*visit_state*/) {
  ...
  ftstop_words_expr *const sw = new ftstop_words_expr(
    v.get_location(),
    stop_words->get_uri(),
...
void end_visit (const FTLanguageOption& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  push_ftstack( new ftlanguage_option_expr( loc, v.get_language() ) );
}
...
void end_visit (const FTStemOption& v, void* /*visit_state*/) {
  ...
  ftstem_option_expr *so =
    new ftstem_option_expr( v.get_location(), v.get_mode() );
  pwo->set_stem_option( so );
}
...
void end_visit (const FTStopWordOption& v, void* /*visit_state*/) {
  ...
  ftstop_word_option_expr::stop_word_list_t stop_words;
  while ( true ) {
    ftexpr *const e = top_ftstack();
    if ( ftstop_words_expr *const sw = dynamic_cast<ftstop_words_expr*>( e ) ) {
  ...
  ftstop_word_option_expr *swo = new ftstop_word_option_expr(
    v.get_location(),
    stop_words,
    v.get_mode()
  );
...
void end_visit (const FTThesaurusID& v, void* /*visit_state*/) {
  TRACE_VISIT_OUT ();
  ftthesaurus_id_expr *const ti = new ftthesaurus_id_expr(
    v.get_location(),
    v.get_uri(),
    v.get_relationship(),
...
void end_visit (const FTThesaurusOption& v, void* /*visit_state*/) {
  ...
  ftthesaurus_option_expr::thesaurus_id_list_t list;
  while ( true ) {
    ftthesaurus_id_expr *const ti =
      dynamic_cast<ftthesaurus_id_expr*>( pop_ftstack() );
    if ( ti )
      list.push_back( ti );
    else
      break;
  }
  ftthesaurus_option_expr *const to = new ftthesaurus_option_expr(
...
void end_visit (const FTWildCardOption& v, void* /*visit_state*/) {
  ...
  ftprimary_with_options_expr *const pwo =
    dynamic_cast<ftprimary_with_options_expr*>( top_ftstack() );
  ftwild_card_option_expr *const wco =
    new ftwild_card_option_expr( v.get_location(), v.get_mode() );
  pwo->set_wild_card( wco );
}
...
void end_visit(const LiteralFunctionItem& n, void* /*visit_state*/)
{
  const QueryLoc& loc = n.get_location();
