    ...
    struct PathHolder* path)
{
  ...
  let_clause_t flref(new let_clause(e->get_sctx(), rCtx.theEM, e->get_loc(), letvar, hoisted));
  ...
  else
  {
    ...
    flwor_expr_t flwor = rCtx.theEM->create_flwor_expr(e->get_sctx(), e->get_loc(), false);
