...
int main(void)
{
  ...
  /*
   * Initialize the zorba server request handler
   */
...
void handleInternalError(int aSignal, siginfo_t *aSignalInfo, void *aContext)
{
  ...
  lMessage << "Signal " << aSignal;
  sendError(lMessage.str());
  exit(aSignal);
}

void handleException(const std::string& aException)
{
  sendError(aException);
  exit(1);
}

void sendError(const std::string& aErrorMessage)
{
  std::string lErrorHeaders = "Status: 500 Internal Server Error\r\n"
                              "Content-Type: application/json\r\n"
                              "\r\n";
  std::string lErrorBody = "{"
                           "  \"source\": \"Internal Error\","
                           "  \"code\": \"INT002\","
                           "  \"message\": \"" + aErrorMessage + "\""
                           "}";

  std::cout << lErrorHeaders << lErrorBody << std::endl;
  std::cerr << lErrorHeaders << lErrorBody << std::endl;
}
