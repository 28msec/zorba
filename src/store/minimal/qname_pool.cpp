    ...
    bool        sync)
{
  ...
  ulong hval = CompareFunction::compute_hash(pre, ln, (uint32_t)(uint64_t)pooledNs.getp());
    ...
    bool                    sync)
{
  ...
  ulong hval = CompareFunction::compute_hash(pre->c_str(), ln->c_str(), (uint32_t)(uint64_t)pooledNs.getp());
