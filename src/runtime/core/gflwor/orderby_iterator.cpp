...
namespace zorba {
  namespace gflwor {
    ...
    void OrderByIterator::matVarsAndOrderBy ( OrderByState* aGroupByState,
        PlanState& aPlanState ) const {
          std::vector<store::Item_t > lItems;
          std::vector<store::TempSeq_t > lSequences;
          
          
          std::vector<ForVarIter_t>::const_iterator lForInputIter = theForVariableInput.begin();
          while(lForInputIter != theForVariableInput.end()){
            
          }
    }

    void OrderByIterator::bindOrderBy ( order_map_t::iterator aOrderMapIter,
