...
{ }
  ...
  template< class Operations>
  Item_t ArithmeticIteratorDurationDateTime<Operations>::nextImpl ( PlanState& planState ) const
  {
    Item_t n0;
    Item_t n1;
    Item_t res;
    ...
    n0 = consumeNext ( this->theChildren[0].getp(), planState );
    if ( n0 != NULL )
    {
      n1 = consumeNext ( this->theChildren[1].getp(), planState );
      if ( n1 != NULL )
      {
