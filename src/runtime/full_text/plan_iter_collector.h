...
#ifndef ZORBA_PLAN_ITER_COLLECTOR_VISITOR_H
#define ZORBA_PLAN_ITER_COLLECTOR_VISITOR_H
...
class PlanIterCollectorVisitor : public ftselection_visitor {
public:
  typedef std::vector<PlanIter_t> PlanIterList_t;

  PlanIterCollectorVisitor() {
  }

  expr_visitor* get_expr_visitor();

  void push_back( PlanIter_t it ) {
    planIterList_.push_back( it );
  }

  PlanIterList_t& getPlanIterList() {
    return planIterList_;
  }
...
private:
  PlanIterList_t planIterList_;
};


} // namespace zorba
#endif  /* ZORBA_PLAN_ITER_COLLECTOR_VISITOR_H */
