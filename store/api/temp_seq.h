...
namespace xqp {
	...
	public:
	 ...
	 /** Info: Tim's territory. Do not touch for the moment!
		 ...
		 * has created iterators for all his bindings he has to produce, it can call purge(). 
		 * After purge() it is not allowed to ask the XDMInstance to create further iterators or items.
		 * Of course it is allowed to use the already created iteratos
		 */
		virtual void purge() = 0;
		
		/** 
		 * Similar to purge(), but in contrast it is still allowed to ask for items or iterators
		 * which start with an item higher than the position number upTo
		 * 
		 ...
		 * which might contain those items.
		 *
		 * @param positions
		 */
