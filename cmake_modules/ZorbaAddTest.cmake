...
MACRO(ZORBA_ADD_TEST_ALLSTORES NAME TARGET ...)
  ...
  FOREACH(ZORBA_STORE_NAME ${ZORBA_STORE_NAMES})
    ...
    IF (WIN32)
      GET_TARGET_PROPERTY(OUTPUT_NAME "${TARGET}${SUFFIX}" OUTPUT_NAME)
      IF (${OUTPUT_NAME} STREQUAL "OUTPUT_NAME-NOTFOUND")
        SET(OUTPUT_NAME "${TARGET}${SUFFIX}")
      ENDIF (${OUTPUT_NAME} STREQUAL "OUTPUT_NAME-NOTFOUND")
      ADD_TEST("${NAME}${SUFFIX}" "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${OUTPUT_NAME}" ${ARGS})
    ELSE (WIN32)
      GET_TARGET_PROPERTY(TARGET_LOCATION "${TARGET}${SUFFIX}" LOCATION)
      ADD_TEST("${NAME}${SUFFIX}" ${TARGET_LOCATION} ${ARGS})
    ENDIF (WIN32)
    SET_TESTS_PROPERTIES ("${NAME}${SUFFIX}" PROPERTIES TIMEOUT "${NORMAL_TIMEOUT_VALUE}")
